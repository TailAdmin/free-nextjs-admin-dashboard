...
export default function ImageUpload({ value, onChange, multiple = true, maxImages = 5 }: ImageUploadProps) {
  ...
  useEffect(() => {
    ...
    // This is a simplified check. In a real app, you might want a more robust comparison
    ...
    return () => {
        ...
        // We don't revoke here immediately because the new state needs the new URLs
        // But we should revoke when the component unmounts
    };
  }, [value]);
      ...
      <div className="space-y-2">
        <div className="flex w-full items-center gap-2 flex-row">
          <Button
            ...
            className={cn(`flex w-full items-center`, images.length > 0 ? 'justify-between !w-full' : 'justify-center', images.length > 0 && badgeVariants({ variant: 'secondary' }))}
            disabled={images.length >= maxImages}
          >
            <Upload className="mr-2 h-4 w-4" />

            <div className="flex w-full items-center justify-center">
              {images.length < 1 && <span>Subir Imágenes</span>}
              {images.length > 0 && (
               
                  <span>{images.length} / {maxImages} archivo{images.length !== 1 ? 's' : ''}</span>
              )}
            </div>
